...
export default async function ReportsPage({
  ...
  return (
    <PageSection>
      <h1 className="text-lg font-semibold">Báo cáo dự án</h1>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {healthCards.map((c) => (
          <Card key={c.label}>
            ...
            <CardContent
              className={`text-xl font-semibold ${c.alert ? "text-destructive" : ""}`}
            >
              {c.value}
            </CardContent>
          </Card>
...
function Burndown({ points }: { points: { date: Date; remaining: number }[] }) {
  ...
  // Ideal line: straight from first remaining down to 0.
